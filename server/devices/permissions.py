...
class IsDeviceOwner(permissions.BasePermission):
    """
    Permission allowing device owners to modify their own devices
    """
...
class IsHomeOwner(permissions.BasePermission):
    """
    Permission allowing home owners to modify their own homes
    """
    def has_object_permission(self, request, view, obj):
...
class IsHomeOwnerOrAdmin(permissions.BasePermission):
    """
    Permission allowing home owners or admins to modify a home
    """
    def has_object_permission(self, request, view, obj):
        return request.user and (
            request.user.role == 'admin' or 
...
class IsHomeMember(permissions.BasePermission):
    """
    Permission allowing home members to access a home
    """
    def has_permission(self, request, view):
        return request.user and request.user.is_authenticated
    
    def has_object_permission(self, request, view, obj):
        # Handle both Room and Home objects
        if hasattr(obj, 'home'):  # For Room objects
            home = obj.home
        else:  # For Home objects
            home = obj
        
        return request.user and (
            home.owner == request.user or 
            request.user in home.members.all() or
