...
// Guest info modal (declared outside GuestsPage)
  ...
  return (
    <div style={{position: 'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'rgba(0,0,0,0.15)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center'}}>
      <div style={{background:'#fff', borderRadius:12, padding:24, minWidth:300, minHeight:180, boxShadow:'0 2px 16px #0002', position:'relative'}}>
        ...
        <h2 style={{fontWeight:600, fontSize:20, marginBottom:12}}>Guest details</h2>
        <div><b>Name:</b> {guest.display_name || '(not provided)'}</div>
  ...
  return (
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="text-center text-muted-foreground mt-8">No guests at the moment.</div>
      ) : (
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mt-8">
               {guests.filter(guest => guest.id !== user?.id).map((guest) => (
                 <div key={guest.id} className="relative bg-white rounded-xl shadow p-6 flex flex-col items-center">
                   <Button className="absolute top-2 right-2 p-1 hover:bg-muted rounded-full" title="Delete" onClick={() => handleDelete(guest.id)} variant="ghost" size="icon">
                     <Trash2 className="w-4 h-4" />
                   </Button>
                   <Button className="absolute top-2 left-2 p-1 hover:bg-muted rounded-full" title="Info" onClick={() => setSelectedGuest(guest)} variant="ghost" size="icon">
                   ...
                   <Button className="absolute bottom-2 right-2 p-1 hover:bg-muted rounded-full" title="Edit" onClick={() => handleEdit(guest)} variant="ghost" size="icon">
